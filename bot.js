...
new DubAPI(config.login, function (apierror, bot) {
                ...
                if (bot.hasPermission(data.user, 'ban')) {
                    ...
                    if (split.length > 1) {
                        ...
                        if (pos !== undefined && bot.getQueue().length >= pos - 1 && !isNaN(pos)) {
                            ...
                            } else bot.sendChat(langfile.error.user_not_in_queue);
                ...
                if (bot.hasPermission(data.user, 'delete-chat')) {
                    ...
                    if (split.length === 2) {
                        User.find({where: {username: {$like: S(split[1]).replaceAll('@', '').s}}}).then(function (user) {
                            ...
                            else bot.sendChat(langfile.error.user_not_found);
                        });
                    } else bot.sendChat(langfile.error.argument);
                }
            }
            ...
            handler: function (data) {
                if (bot.hasPermission(data.user, 'queue-order')) {
                    ...
                    bot.getQueue().forEach(function (queueobject) {
                        setTimeout(function () {
                            bot.moderatePauseDJ(queueobject.user.id);
                        }, _.random(2, 10) * 1000);
                    });
                }
            }
    ...
    function removeafk() {
        User.findAll({where: {warned_for_afk: true}}).then(function (users) {
            ...
            users.forEach(function (user) {
                if (_.contains(queue, bot.getUser(user.userid))) {
                    afks.push('@' + user.username);
                    removeUserFromQueue(user.userid, config.afkremoval.action);
    ...
    function checkQueue(queue) {
        queue.forEach(function (queueobject, index) {
            if (queueobject.media.songLength > config.autoskip.timelimit.limit * 1000 && config.autoskip.timelimit.enabled && toggle.songlength) {
                removeUserFromQueue(queueobject.user.id, config.queuecheck.action);
                Reputation.create({
    ...
    function removeUserFromQueue(userid, action) {
        ...
        else console.log('You misconfigured an action in your config! Please check!');
    }
});
